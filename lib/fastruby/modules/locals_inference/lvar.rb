...
module FastRuby
  class LocalsInference
    ...
    define_process_for(:lvar) do |tree|
    ...
    define_process_for(:self) do |tree|
      next tree unless @infer_self
      
      fs(:call, tree, :infer, fs(:arglist, fs(:const, @infer_self.to_s.to_sym)))
    end
  end
end
